...
var styleSwitcher = {
    ...
    initialize: function () {
        ...
        $("head").append($('<link rel="stylesheet">').attr("href", "static/assets/plugins/style-switcher/style-switcher.css"));
        $this.build();
        $this.events();
...
//        }
    ...
    build: function () {
        ...
        $.each(patterns, function (i, value) {
            var pattern = $("<li />").append($("<a />")
                    .css({
                        "background": 'url("static/assets/img/patterns/' + patterns[i].patternImage + '.png") repeat 50% 50%' + patterns[i].Hex
                    })
                    .attr({
    ...
    setColor: function (color) {
        ...
        $colorSwitcherLink.attr('href', 'static/assets/css/multicolors/theme-' + color + '.css');
        $.cookie("skin", color);
        $this.updatePatternColor();
    },
    ...
    setPattern: function (pattern) {
        $('body.boxed').css({"background-image": 'url("static/assets/img/patterns/' + pattern + '.png")'});
        $.cookie("pattern", pattern);
    },
    updatePatternColor: function () {
        $('body.boxed').css({"background-image": 'url("static/assets/img/patterns/' + $.cookie("pattern") + '.png")'});
        ...
        patternListToUpdate.each(function () {
            ...
            //$(this).css({"background-image": 'url("static/assets/img/patterns/' + $(this).attr('data-image') + '_' + $.cookie("skin") + '.png")'});
            $(this).css({"background-image": 'url("static/assets/img/patterns/' + $(this).attr('data-image') + '.png")'});
        });
    },
    reset: function () {
        $.removeCookie("skin");
